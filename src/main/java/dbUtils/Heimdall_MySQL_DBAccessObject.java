...
@Slf4j
public class Heimdall_MySQL_DBAccessObject extends DBAccessObject {
    ...
    public static synchronized Connection getMySqlDbConnection() throws SQLException, ClassNotFoundException {
        String dbUrl;
        if (conn == null) {
            String dbName = ReadProperties.testConfigMap.get(UtilConstants.HEIMDALL_DBNAME).toString();
            String dbUserName = ReadProperties.testConfigMap.get(UtilConstants.HEIMDALL_USER).toString();
            String dbPassword = ReadProperties.testConfigMap.get(UtilConstants.HEIMDALL_HPASSWORD).toString();
            String dbHost = ReadProperties.testConfigMap.get(UtilConstants.HEIMDALL_HOST).toString();
            int dbPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.HEIMDALL_PORT).toString());
            ...
            Heimdall_MySQL_DBAccessObject.doSshTunnel(sshUser, sshPassword, sshHost, sshPort, dbHost, localPort, dbPort);
