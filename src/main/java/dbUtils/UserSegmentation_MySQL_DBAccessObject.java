...
@Slf4j
public class UserSegmentation_MySQL_DBAccessObject extends DBAccessObject {
    ...
    public static synchronized Connection getMySqlDbConnection() throws SQLException, ClassNotFoundException {
        ...
        if (conn == null) {
            String dbName = ReadProperties.testConfigMap.get(UtilConstants.USERSEGMENTATION_DBNAME).toString();
            String dbUserName = ReadProperties.testConfigMap.get(UtilConstants.USERSEGMENTATION_USER).toString();
            String dbPassword = ReadProperties.testConfigMap.get(UtilConstants.USERSEGMENTATION_HPASSWORD).toString();
            String dbHost = ReadProperties.testConfigMap.get(UtilConstants.USERSEGMENTATION_HOST).toString();
            int dbPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.USERSEGMENTATION_PORT).toString());
            ...
            UserSegmentation_MySQL_DBAccessObject.doSshTunnel(sshUser, sshPassword, sshHost, sshPort, dbHost, localPort, dbPort);
