...
@Slf4j
public class ams_MySQL_DBAccessObject extends DBAccessObject{
    ...
    public static synchronized Connection getMySqlDbConnection() throws SQLException, ClassNotFoundException {
        String dbUrl;
        if (conn == null) {
            String dbName = ReadProperties.testConfigMap.get(UtilConstants.AMS_DBNAME).toString();
            String dbUserName = ReadProperties.testConfigMap.get(UtilConstants.AMS_USER).toString();
            String dbPassword = ReadProperties.testConfigMap.get(UtilConstants.AMS_HPASSWORD).toString();
            String dbHost = ReadProperties.testConfigMap.get(UtilConstants.AMS_HOST).toString();
            int dbPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.AMS_PORT).toString());
            ...
            if(System.getProperty("env").equals("local")) {
                ...
                ams_MySQL_DBAccessObject.doSshTunnel(sshUser, sshPassword, sshHost, sshPort, dbHost, localPort, dbPort);
