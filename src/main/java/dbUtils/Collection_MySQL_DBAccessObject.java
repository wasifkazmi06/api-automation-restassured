...
@Slf4j
public class Collection_MySQL_DBAccessObject extends DBAccessObject {
    ...
    public static synchronized Connection getMySqlDbConnection() throws SQLException,ClassNotFoundException {
        String dbUrl;
        if(conn == null){
            String dbName = ReadProperties.testConfigMap.get(UtilConstants.JUSTPAY_DBNAME).toString();
            String dbUserName = ReadProperties.testConfigMap.get(UtilConstants.JUSTPAY_USER).toString();
            String dbPassword = ReadProperties.testConfigMap.get(UtilConstants.JUSTPAY_PASSWORD).toString();
            String dbHost =ReadProperties.testConfigMap.get(UtilConstants.JUSTPAY_HOST).toString();
            int dbPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.JUSTPAY_PORT).toString());
...
//
//
//                Collection_MySQL_DBAccessObject.doSshTunnel(sshUser, sshPassword, sshHost, sshPort, dbHost, localPort, dbPort);
