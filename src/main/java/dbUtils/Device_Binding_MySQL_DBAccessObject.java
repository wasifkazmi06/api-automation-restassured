...
@Slf4j
public class Device_Binding_MySQL_DBAccessObject extends DBAccessObject{
    ...
    public static synchronized Connection getMySqlDbConnection() throws SQLException, ClassNotFoundException {
        String dbUrl;
        if (conn == null) {
            String dbName = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_DBNAME).toString();
            String dbUserName = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_USER).toString();
            String dbPassword = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_HPASSWORD).toString();
            String dbHost = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_HOST).toString();
            int dbPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_PORT).toString());
            ...
            if(System.getProperty("env").equals("local")) {
                ...
                String sshUser = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_SSH_USERNAME).toString();
                String sshPassword = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_SSH_PASSWORD).toString();
                int sshPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_SSH_PORT).toString());
                int localPort = Integer.parseInt(ReadProperties.testConfigMap.get(UtilConstants.SSH_LOCAL_PORT).toString());
                String sshHost = ReadProperties.testConfigMap.get(UtilConstants.DEVICE_BINDING_SSH_HOST).toString();

                Device_Binding_MySQL_DBAccessObject.doSshTunnel(sshUser, sshPassword, sshHost, sshPort, dbHost, localPort, dbPort);
